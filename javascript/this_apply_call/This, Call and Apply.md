...
Excepting the uncommonly situation like `with` and `eval,` there is four standard status of `this` pointer.
...
We can see that the `this` in `callback()` is pointing to `window`. One way to solve this problem is using one variable `that ` to keep the reference of `this`, which points to `div1`. Another way is **Arrow function**,we will analyze it in the following section.
...
The only difference between `Call` and `Apply` is the format of the passed arguments. The format of passed arguments in `Apply` is *array*, and argument number in `Call` is not fixed. 
...
If we pass the `null` as the first parameter in `Apply` or `Call`, the `this` in function will point to global object.
